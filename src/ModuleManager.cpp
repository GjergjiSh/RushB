...
int ModuleManager::Init()
{
    ...
    if (m_parameter_manager->Load_Configuration() != 0) return -1;
...
int ModuleManager::Init_Module(Module *const &module) {
    ...
    m_logger.Time_Info(module->name.append(" initialized in "), (int) elapsed.count());
...
void *ModuleManager::Get_Module_Handle(const std::filesystem::directory_entry &entry) {// Load the library
...
Module *ModuleManager::Create_Module(void *lib_handle) {// load the factory method of the Module
